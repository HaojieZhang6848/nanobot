...
class ToolDefinitionProvider(Protocol):
    def get_definitions(self) -> list[dict[str, Any]]:
        """Return tool definitions passed to the model request."""
        ...
...
class ContextGovernor:
    ...
    def prepare_for_model(
        ...
        state: ContextGovernanceState,
    ) -> list[dict[str, Any]]:
        updated = self.drop_orphan_tool_results(messages)
        updated = self.backfill_missing_tool_results(updated)
        updated = self.apply_tool_result_budget(config, updated)
        updated = self.compact_inflight_overflow(config, updated, state)
    ...
    def compact_inflight_overflow(
        ...
        updated = self._apply_recorded_compactions(messages, state)
        ...
        candidates = self._inflight_compaction_candidates(config, updated, state)
        if not candidates:
            return updated

        for idx, tool_call_id in candidates:
            if tool_call_id in state.compacted_tool_call_ids:
                continue
            if updated is messages:
                updated = [dict(m) for m in messages]
            state.compacted_tool_call_ids.add(tool_call_id)
        ...
        logger.debug(
            ...
            target,
            source,
            len(state.compacted_tool_call_ids),
        )
        return updated
    ...
    def _legal_history_tail(
        ...
        if kept:
            kept = self._trim_to_first_user(kept) or self._nearest_user_tail(non_system) or kept
        else:
            kept = self._nearest_user_tail(non_system) or (non_system[-1:] if non_system else [])
    ...
    @staticmethod
    def _nearest_user_tail(messages: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    def _apply_recorded_compactions(
        ...
        if not state.compacted_tool_call_ids:
            return messages
        updated = messages
        for idx, msg in enumerate(messages):
            ...
            if not tool_call_id or str(tool_call_id) not in state.compacted_tool_call_ids:
                continue
    ...
    def _inflight_compaction_candidates(
        ...
        for idx, msg in enumerate(messages):
            ...
            if msg.get("role") != "tool" or msg.get("name") not in COMPACTABLE_TOOLS:
                continue
            tool_call_id = msg.get("tool_call_id")
            if not tool_call_id or str(tool_call_id) in state.compacted_tool_call_ids:
                continue
        ...
        # Hard overflow beats the keep-recent preference, but preserve the
        # newest result as the last resort before history snipping.
        fallback = compactable[primary_count:-1]
