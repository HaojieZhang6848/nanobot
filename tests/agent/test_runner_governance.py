...
@pytest.mark.asyncio
async def test_microcompact_skips_when_prompt_under_hard_budget(monkeypatch):
...
@pytest.mark.asyncio
async def test_microcompact_overflow_compacts_to_low_watermark(monkeypatch):
    ...
    def estimate(_provider, _model, msgs, _tools):
        ...
        for msg in msgs:
            ...
            if content == long_content:
                tokens += 100
            elif isinstance(content, str) and "omitted from context" in content:
                tokens += 1
        return tokens, "test"
...
@pytest.mark.asyncio
async def test_context_governor_keeps_compaction_boundary_stable(monkeypatch):
    provider = MagicMock()
    provider.generation = SimpleNamespace(max_tokens=0)
    tools = MagicMock()
    tools.get_definitions.return_value = []
    ...
    def estimate(_provider, _model, msgs, _tools):
        ...
        for msg in msgs:
            if msg.get("role") != "tool":
                continue
            content = msg.get("content")
            tokens += 100 if content == long_content else 1
        return tokens, "test"
    ...
    first = governor.compact_inflight_overflow(config, messages, state)
    first_ids = set(state.compacted_tool_call_ids)

    second = governor.compact_inflight_overflow(config, messages, state)

    assert state.compacted_tool_call_ids == first_ids
    assert [m.get("content") for m in second] == [m.get("content") for m in first]


@pytest.mark.asyncio
async def test_microcompact_preserves_short_results(monkeypatch):
...
@pytest.mark.asyncio
async def test_microcompact_skips_non_compactable_tools(monkeypatch):
